...
// const GC_BSC_OMNIBRIDGE = networks[BSC_CYBA_BRIDGE].foreignMediatorAddress;
const GC_BSC_OMNIBRIDGE = networks[ETH_CYBA_BRIDGE].foreignMediatorAddress;
...
export const useTokenGCOriginOnBSC = token => {
  ...
  const load = useCallback(async () => {
    setFetching(true);
    try {
      if (token && token.chainId === 1 && bridgeDirection === ETH_CYBA_BRIDGE) {
