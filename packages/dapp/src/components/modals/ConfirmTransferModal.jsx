...
export const ConfirmTransferModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const b = await ethersProvider.getBalance(account);
        setBalance(b);
      } catch (error) {
        logError(`Error fetching balance for ${account}`, error);
      }
    })();
  }, [
    ethersProvider,
      ...
      <ModalOverlay background="modalBG">
        ...
        >
          ...
          <ModalBody px={6} py={0}>
            ...
            <Flex align="center" fontSize="sm" justify="center" mt={4}>
              {`Balance ${formatValue(balance, 18)} ${getNativeCurrency(fromToken.chainId).symbol}`}
            </Flex>
