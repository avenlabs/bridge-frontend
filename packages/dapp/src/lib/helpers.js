...
export const uriToHttp = uri => {
  ...
  switch (protocol) {
    ...
    case 'local':
      return [uri.replace('local:', process.env.REACT_APP_URL)]
    default:
      return [];
  }
};
...
export const getRPCKeys = bridgeDirection => {
  switch (bridgeDirection) {
    // case BSC_CYBA_BRIDGE:
    //   return {
    //     homeRPCKey: CYBA_RPC_URL,
...
export const getHelperContract = (chainId, homeChainId) => {
  return nativeCurrencyMediators[chainId || 1][homeChainId];
}
